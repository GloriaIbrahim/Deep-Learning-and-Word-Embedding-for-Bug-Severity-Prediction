...
def GRUmodel(doc2vecmodel):
    model = Sequential()
    model.add(GRU(64, input_shape=(500, 1), dropout=0.4, recurrent_dropout=0.4, return_sequences=True))
...
def applyGRUonMozilla(MozillaVectorsTrain, MozillaVectorsTest, MozillaLabelTrain, MozillaLabelTest, MozillaDoc2VecModel):
    ...
    MozillaGRUModel.fit(MozillaVectorsTrain, MozillaLabelTrain, batch_size=100, epochs=20)

    predictions = MozillaGRUModel.predict_classes(MozillaVectorsTest)
    print(predictions[0])
    print(MozillaLabelTest[0])
    ...
    MozillaGRUModel.save('ClassifiersModels/GRU_mozilla_model_keras.h5')
...
def applyGRUonEclipse(EclipseVectorsTrain, EclipseVectorsTest, EclipseLabelTrain, EclipseLabelTest, EclipseDoc2VecModel):
    ...
    EclipseGRUModel.fit(EclipseVectorsTrain, EclipseLabelTrain, batch_size=100, epochs=20)
    ...
    EclipseGRUModel.save('ClassifiersModels/GRU_eclipse_model_keras.h5')
...
def applyGRUonBoth(BothVectors, BothLabelEncoded, BothDoc2VecModel):
    ...
    BothGRUModel.fit(BothVectors, BothLabelEncoded, batch_size=100, epochs=20)

    BothGRUModel.save('ClassifiersModels/GRU_both_model_keras.h5')
